...
rownames(lineaCod) = as.character(lineaCod$COD_VARIAN)

# Crea variables de interes y agrupa 
datosDia = datos %>% filter(Dia == dia) %>%
  mutate(
    # timeVrec = ifelse(timeViaje <= 15*60,'0 a 15',
  ...
  group_by(sentido_variante_subida,
           ...
           codigo_parada_bajada,Dia,minutos_sub,minutos_baj) %>% #,minutos_sub,minutos_baj) %>%
  summarise(Tot = n()) %>%
  ungroup() %>% filter(Tot >= 0) %>%
  mutate(ID_SubBaj = paste(sevar_codigo_subida,codigo_parada_subida,codigo_parada_bajada,sep = '_'),
         ID_Sub = paste(sevar_codigo_subida,codigo_parada_subida,sep = '_'),
         ID_Baj = paste(sevar_codigo_subida,codigo_parada_bajada,sep = '_')) %>% 
  ...
  select(-c(CODSEC_baj:MUNICIPIO_sub)) %>% 
  rename('COD_VARIAN' = 'sevar_codigo_subida') %>% 
  mutate(COD_VARIAN = as.character(COD_VARIAN),
         ID_Unico = row_number()) %>%
  suppressMessages()
...
####################################################
####### Arma la secuencia maxima de cada variante

##### Identifica la cantidad combinaciones a genera (coincide con el numero de filas)
trials = 1:length(lineasUsar)

##### Funcion que itera de forma paralela
boot_fx = function(jj) {
  
  codLinVar = lineasUsar[jj]
  DESC_LINEA = as.character(lineaCod[codLinVar,'DESC_LINEA'])
  
  res = getSecuenciaAllLinVar(codLinVar = codLinVar,
                              listLineaProy,
                              puntList,
                              paradas) 
...
clusterExport(cl, c("paradas","lineasUsar","lineaCod","getSecuenciaAllLinVar","listLineaProy",
                    "puntList","getSecuencia","cargaCoords"))
clusterEvalQ(cl, {
  library(tidyverse)
  library(sf)
  library(mefa)})
...
resFinJoin = resFin %>% filter(COD_UBIC_P != 0) %>%
  mutate(ID_VarPar = paste(COD_VARIAN,COD_UBIC_P,sep = '_'))



datosDiaUnic = datosDia %>% group_by(COD_VARIAN,ID_SubBaj,ID_Sub,ID_Baj) %>% 
  summarise(n = n()) %>% 
  ungroup() %>% 
  left_join(resFinJoin %>% select(ID_VarPar,seqViajeN), 
            by = c('ID_Sub' = 'ID_VarPar')) %>%
  rename('seqViaje_Sub' = 'seqViajeN') %>%
  left_join(resFinJoin %>% select(ID_VarPar,seqViajeN), 
            by = c('ID_Baj' = 'ID_VarPar')) %>%
    rename('seqViaje_Baj' = 'seqViajeN') %>%
  mutate(n_expand = seqViaje_Baj - seqViaje_Sub) 


datosDiaUnicDep = datosDiaUnic %>% filter(!is.na(n_expand) & n_expand > 0)

datosDiaExpand = datosDiaUnicDep %>% uncount(n_expand) %>%
  group_by(ID_SubBaj) %>% mutate(seqViajeN = row_number() + seqViaje_Sub - 1) %>%
  left_join(resFin %>% select(COD_VARIAN,seqViajeN,distAnt),by = c('COD_VARIAN','seqViajeN'))


sum(!datosDiaUnic$ID_Sub %in% resFinJoin$ID_VarPar)
sum(!datosDiaUnic$ID_Baj %in% resFinJoin$ID_VarPar)


########################################
#### Expande la base para generar los viajes

datFin = datosDia %>% left_join(datosDiaExpand %>% select(ID_SubBaj,seqViajeN)) 

datFin = datFin %>% left_join(resFin %>% select(COD_VARIAN,seqViajeN,distAnt,distAnt,ID_punto), 
                              by = c('COD_VARIAN','seqViajeN')) %>%
  rename(c('DESC_VARIA' = 'sentido_variante_subida',
           'par_Sub' = 'codigo_parada_subida',
           'par_Baj' = 'codigo_parada_bajada'))




names(resPoint)[!names(resPoint) %in% names(datFin)]
# datosDiaUnic %>% filter(n_expand <= 3)
# summary(datosDiaUnic$n_expand)

datosDiaUnicDep %>% filter(n_expand <= 0)
summary(datosDiaUnicDep$n_expand)


puntRefUnicos = resFin %>% select(ID_calle,NOM_CALLE,COD_NOMBRE,direction,X,Y,dirAv,ID_punto) %>%
  group_by(ID_punto) %>% slice_head() %>% 
  ungroup() %>%
  st_as_sf(x=.,coords = c('X','Y'),crs = 32721)  %>% 
  st_transform(.,crs = 4326) %>% cargaCoords()
